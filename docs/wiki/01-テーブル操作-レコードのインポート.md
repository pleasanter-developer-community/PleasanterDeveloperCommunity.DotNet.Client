...
// ファイルパス版
Task<ApiResponse<ImportResponse>> ImportFromFileAsync(long siteId, string filePath, Encoding? encoding = null, string? key = null, bool? migrationMode = null, bool autoConvertToUtf8 = false, TimeSpan? timeout = null)
...
### リクエストオブジェクト使用版 (`ImportAsync`)

| 引数 | 型 | 必須 | 説明 |
|------|------|:----:|------|
| `siteId` | long | ✓ | サイトID |
| `request` | ImportRequest | ✓ | インポートリクエストオブジェクト |
| `csvData` | byte[] | ✓ | CSVファイルのバイナリデータ（byte[]版） |
| `csvStream` | Stream | ✓ | CSVファイルのストリーム（Stream版） |
| `fileName` | string | ✓ | ファイル名 |
| `timeout` | TimeSpan? | | リクエストタイムアウト |
