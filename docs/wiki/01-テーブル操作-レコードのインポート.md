...
| `encoding` | Encoding? | | CSVファイルのエンコーディング（`Encoding.UTF8` または `Encoding.GetEncoding(932)`、デフォルト: `Encoding.UTF8`）。`ImportFromFileAsync`では省略時に自動判定されます。 |
| `key` | string? | | キーが一致するレコードを更新する場合のキー項目（例: IssueId, ClassA など）。指定すると自動的に更新モードになります。 |
| `migrationMode` | bool? | | 移行モードでのインポートを実施する場合はtrue |
| `autoConvertToUtf8` | bool | | `ImportFromFileAsync`専用。UTF-8/Shift-JIS以外のエンコーディングが指定された場合、自動的にUTF-8に変換する（デフォルト: false） |
