# 添付ファイル取得 - binaries/{guid}/get <!-- omit in toc -->
...
```csharp
// パラメータ指定版
Task<ApiResponse<AttachmentResponse>> GetAttachmentAsync(string guid, TimeSpan? timeout = null, CancellationToken cancellationToken = default)

// リクエストモデル版
Task<ApiResponse<AttachmentResponse>> GetAttachmentAsync(string guid, GetAttachmentRequest request, TimeSpan? timeout = null, CancellationToken cancellationToken = default)
```
