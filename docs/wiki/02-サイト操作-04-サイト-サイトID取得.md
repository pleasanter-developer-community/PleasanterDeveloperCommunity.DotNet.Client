...
### 通常版 (`GetClosestSiteIdAsync`)

| 引数 | 型 | 必須 | 説明 |
|------|------|:----:|------|
| `siteId` | long | ✓ | サイトID（検索を開始するサイトのID） |
| `findSiteNames` | List\<string\> | ✓ | 検索対象のサイト名の配列 |
| `timeout` | TimeSpan? | | リクエストタイムアウト |

### リクエストオブジェクト使用版 (`GetClosestSiteIdAsync`)

| 引数 | 型 | 必須 | 説明 |
|------|------|:----:|------|
| `siteId` | long | ✓ | サイトID（検索を開始するサイトのID） |
| `request` | GetClosestSiteIdRequest | ✓ | サイトID取得リクエストオブジェクト |
| `timeout` | TimeSpan? | | リクエストタイムアウト |

## GetClosestSiteIdRequest クラス

| プロパティ | 型 | 必須 | 説明 |
|------|------|:----:|------|
| `ApiVersion` | float | | APIバージョン（既定値: 1.1） |
| `ApiKey` | string | ✓ | APIキー（自動設定） |
| `FindSiteNames` | List\<string\> | ✓ | 検索対象のサイト名の配列 |
...
{
    FindSiteNames = new List<string> { "ParentSite", "HideSite" }
};

var response = await client.GetClosestSiteIdAsync(siteId: 12345, request: request);
...
- [開発者向け機能：API：サイト操作：サイト名検索で該当サイトに最も近いサイトID取得](https://pleasanter.org/ja/manual/api-site-get-closest-siteid)
