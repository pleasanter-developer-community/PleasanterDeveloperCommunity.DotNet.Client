...
var request = new ExportRequest
{
    ...
    View = new View
    {
        ColumnSorterHash = new Dictionary<string, SortOrderType>
        {
            { "CreatedTime", SortOrderType.Asc }
        }
    }
};

var result = await client.ExportAsync(siteId: 123, request: request);
```
...
| `Type` | string | 出力するファイルの種類（"csv" または "json"） |
...
- [公式マニュアル: テーブルのエクスポート](https://pleasanter.org/ja/manual/api-export)
- [公式マニュアル: テーブルの管理：エクスポート](https://pleasanter.org/ja/manual/table-management-export)
