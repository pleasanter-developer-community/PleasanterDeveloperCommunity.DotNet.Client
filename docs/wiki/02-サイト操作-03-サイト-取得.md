...
// リクエストオブジェクト使用版
Task<ApiResponse<GetSiteResponse>> GetSiteAsync(long siteId, GetSiteRequest request, TimeSpan? timeout = null)
```
...
### リクエストオブジェクト使用版 (`GetSiteAsync`)

| 引数 | 型 | 必須 | 説明 |
|------|------|:----:|------|
| `siteId` | long | ✓ | サイトID（取得対象のサイトのID） |
| `request` | GetSiteRequest | ✓ | サイト取得リクエストオブジェクト |
| `timeout` | TimeSpan? | | リクエストタイムアウト |
...
if (result.IsSuccess)
{
    ...
    // サイト設定はオブジェクトとして取得されます
    if (site?.SiteSettings != null)
    {
        Console.WriteLine($"サイト設定: {site.SiteSettings}");
    }
}
```
